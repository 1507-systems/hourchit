...
The shape this is designed for: a tenant with a single institutional customer
that will not be given a client portal. Bookings, changes, and cancellations
arrive by **email** from a couple of named people, and invoices go out by email
to a different couple of named people. So the coordination surface is a mailbox,
not a login, and HourChit has to be the thing that reads and writes it.

Real tenant and contact names live in the private tenants repo. Everything below
uses the documented example tenant, **Example Contracting LLC**, and its
customer, **Example Institutional Client** — the same pair `profiles/example.json`
describes.

The tenant still gets a portal — their own. Inbound mail lands there, and their
calendar subscribes to what they confirm.
...
|---|---|---|
| Booking and cancellation | Dana Reyes, Terry Hale | 2.5 |
| Accounts payable | Robin Vale, Sam Okafor | 5.2 |
...
```
<tenant>@hosted.hourchit.app      e.g. example@hosted.hourchit.app
```
...
go on holiday, and when Dana hands scheduling to somebody else the tenant must
be able to fix it themselves at 7pm without a deploy. So contacts are a managed table
with a screen: add, edit, deactivate, change role.

**Deactivate, never delete.** A cancellation email from Dana in March must still
resolve to Dana in December, after she has left. Deleting the row orphans the
...
One person can hold more than one role: a §2.5 booking contact is often also
the signatory on the SOW, and that is normal rather than a modelling error.
...
| Kind | To | CC | BCC |
|---|---|---|---|
| Invoice | AP contacts marked primary | remaining AP contacts | the tenant |
| Booking confirmation | the contact who requested it | other booking contacts | the tenant |
| Cancellation confirmation | the contact who cancelled | other booking contacts | the tenant |
| Change order | booking contacts | — | the tenant |

Two deliberate choices in that table:

**BCC to the tenant on everything.** They get a copy in their own mail without
the customer seeing a self-addressed message, so the record survives even if the
app does. This is the cheap insurance against HourChit being the only place a sent
invoice exists.

**Other contacts in the role are CC'd, not omitted.** SOW 2.5 says *either*
booking contact may bind the customer. If Terry cancels and Dana never sees the
confirmation, the customer's own left hand is uninformed and the dispute that
follows is one the tenant has to win with records. Copying the role costs
nothing.
...
The setup memo tells the tenant to collect a specific pile of paper from an
institutional customer: a conflict-of-interest policy or disclosure form, a
vendor onboarding packet, a W-9 request, their insurance requirements, purchase
orders. It also tells them that anything agreed verbally should be followed up
in writing. Those
...
into R2. The tenant already has WorkDrive, already has the app on their phone, and
...
| Artifact | Home | Why |
|---|---|---|
| Client documents, invoice PDFs, inbound attachments | WorkDrive, per-client folder | the tenant's, phone-accessible, outlives the app |
...
7. **Surface** — thread and draft appear in the tenant's portal for approval.
...
The same reasoning applies to the verbal-notice path in SOW 3.4: when a booking
contact cancels by phone, the tenant records it and HourChit sends the confirming
...
When a booking is confirmed, HourChit emails the tenant the event as an `.ics`
...
a native invitation. That is acceptable, and arguably correct here: the tenant
does not need to RSVP to their own bookings. The organiser/attendee machinery exists for
...
contact_roles   contact_id, role('booking'|'ap'|'signatory'), is_primary
                -- many-to-many: Dana is both a booking contact and the signatory
...
- **Idempotent sends.** Invoice *n* has one idempotency key. A retry, a double
  click, or a queue redelivery must not produce two invoices at an
  institutional customer's accounts payable.
...
- **A visible outbox.** Queued, sent, failed, with the error and a retry button.
  If the tenant cannot see that Tuesday's invoice failed, the system has not
  told them anything.
