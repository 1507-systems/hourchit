...
`customers` → `tasks` (rate per task) → `time_entries` (event name in `note`,
unbilled until an `invoice_id` is set). `routes` feed `mileage_entries`
(billable flag + reason, rate captured per row). `invoices` store subtotals +
total in cents and a period derived from the billed rows. Invoice lines snapshot
the service label, event name, and local service date so an issued invoice is
historically stable. Time entries aggregate only when service, event name,
local date, rate, and applicable terms all match. A partial unique index
enforces **at most one running timer** at the database level.
