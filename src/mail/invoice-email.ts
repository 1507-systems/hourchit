 ...
 *
 * A client of Example Contracting receives an invoice from hosted.hourchit.app -- a
 ...
 *
 * The wording follows the convention used by white-labelled booking services:
 * name the business in BOLD first, because that is who the
...
/**
 * The one line above the invoice itself.
 *
 * The invoice stays the body of the email -- this is not a substitute for it,
 * just the sentence a reader expects before a wall of line items so the
 * message doesn't open straight into a table.
 */
function invoiceEmailIntro(v: InvoiceEmailView): string {
  return `Hi ${v.customer.name}, please find your invoice from ${v.business.name} below, with a PDF copy attached for your records.`;
}

export function invoiceEmailSubject(v: { invoice: Invoice; business: { name: string } }): string {
...
export function invoiceEmailText(v: InvoiceEmailView): string {
  ...
  for (const l of v.lines) {
    ...
    out.push(`${text.primary}${detail}`);
    if (text.secondary) out.push(`  ${text.secondary}`);
    out.push(`    ${money(l.amount_cents)}`);
  }
...
export function invoiceEmailHtml(v: InvoiceEmailView): string {
  ...
  const rows = v.lines
    .map((l) => {
      ...
      return `<tr>
      <td style="${CELL}">
        <div style="font-size:15px;font-weight:700">${esc(text.primary)}</div>
        ${
          text.secondary
            ? `<div style="color:#656d76;font-size:13px;margin-top:2px">${esc(text.secondary)}</div>`
            : ''
        }
      </td>
  ...
  <p style="margin:0 0 20px;font-size:15px">${esc(invoiceEmailIntro(v))}</p>
