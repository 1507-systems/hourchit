...
/** A UTC instant, `minutes` later -- for turning a manual duration into a stoppedAt. */
export function addMinutesIso(startedAtIso: string, minutes: number): string {
  const start = Date.parse(startedAtIso);
  if (Number.isNaN(start)) {
    throw new Error(`addMinutesIso: invalid timestamp ${JSON.stringify(startedAtIso)}`);
  }
