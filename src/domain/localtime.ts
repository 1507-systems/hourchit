...
/**
 * Split one attendance across the local calendar dates it actually touched.
 * The already-rounded billable total is distributed proportionally, so a
 * call-out minimum or billing increment is applied once to the attendance,
 * never once per side of midnight.
 */
export function splitBillableSecondsByLocalDate(
  startedAt: string,
  stoppedAt: string,
  timeZone: string,
  ...
  if (!Number.isFinite(startMs) || !Number.isFinite(stopMs) || stopMs <= startMs) {
    throw new Error('splitBillableSecondsByLocalDate: attendance must have a positive duration');
  }

  const rawParts: Array<{ serviceDate: string; milliseconds: number }> = [];
  let cursor = startMs;
  while (cursor < stopMs) {
    const serviceDate = localDateString(new Date(cursor).toISOString(), timeZone);
    const nextDate = new Date(`${serviceDate}T00:00:00.000Z`);
    nextDate.setUTCDate(nextDate.getUTCDate() + 1);
    const nextLocalMidnight = Date.parse(
      zonedWallTimeToUtc(`${nextDate.toISOString().slice(0, 10)}T00:00`, timeZone),
    );
    const end = Math.min(stopMs, nextLocalMidnight);
    rawParts.push({ serviceDate, milliseconds: end - cursor });
    cursor = end;
  }

  const totalMilliseconds = stopMs - startMs;
  let cumulativeMilliseconds = 0;
  let previouslyAllocated = 0;
  return rawParts.map((part) => {
    cumulativeMilliseconds += part.milliseconds;
    const allocated = Math.round((billableSeconds * cumulativeMilliseconds) / totalMilliseconds);
    const seconds = allocated - previouslyAllocated;
    previouslyAllocated = allocated;
    return { serviceDate: part.serviceDate, seconds };
  });
}
