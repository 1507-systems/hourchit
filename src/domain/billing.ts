 ...
 * increment, and the example tenant's SOW 3.2, which bills in fifteen minute
 * increments rounded up to the next increment.
 *
 * THE MINIMUM APPLIES PER ATTENDANCE, NOT PER INVOICE. The example tenant's
 * MSA 1.5 makes each confirmed attendance billable at the minimum in its own
 * right. Three separate twenty minute
 ...
 *
 * The example tenant's SOW 3.3 is the motivating case: a four hour minimum
 * per confirmed attendance for any event scheduled to begin on a Saturday or
 * Sunday, and no minimum at all for one scheduled to begin Monday through
