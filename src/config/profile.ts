...
import type { InvoiceDeliveryMode } from '../domain/invoice-delivery';
...
export interface ProfileSettings {
   ...
   * tenant's terms leak into another's simply by being the fallback. The
   * example tenant's SOW 3.2 and the 1507 break-fix card both use 15.
   */
   ...
   * the notice period is a negotiated SOW term. In the example tenant's
   * paperwork, SOW section 3.6 is a per-SOW field, and one client's SOW names
   * sixty days where another names something else.
   *
   ...
   *
   * MIND THE CONTRACTUAL FLOOR when setting it. A real agreement can define
   * the floor relationally rather than as a number: the example tenant's MSA
   * section 3.3 requires notice of a standard rate update to exceed the
   * termination-for-convenience period of section 2.2 by thirty days, and with
   * that period at thirty days, sixty is the minimum the agreement permits
   * rather than a preference. A tenant on a different MSA has a different
   * floor.
   */
...
export interface SeedCustomer {
  ...
  email: string;
  /**
   * OPTIONAL, defaulting to 'hosted' to match the column's own default --
   * but the generator writes it explicitly either way, rather than leaving
   * it to SQLite. A seeded client is a real client from day one, and
   * `invoice_delivery_mode` is meant to be a decision someone made, not
   * whatever a schema default happened to be when this profile was written.
   */
 ...
 */
export function termsFor(profile: TenantProfile): BillingTerms {
  return {
