...
import {
  ...
  INVOICE_DELIVERY_MODE_LABELS,
  invoiceDeliveryModes,
  type InvoiceDeliveryMode,
} from '../domain/invoice-delivery';

function deliveryModeOptions(selected: InvoiceDeliveryMode): string {
  return invoiceDeliveryModes
    .map(
      (m) =>
        `<option value="${m}"${m === selected ? ' selected' : ''}>${esc(INVOICE_DELIVERY_MODE_LABELS[m])}</option>`,
    )
    .join('');
}
...
export function renderClients(
  ...
  /** This tenant's current default for new clients; preselects the add-client form. */
  tenantDefaultMode: InvoiceDeliveryMode,
  flash = '',
): string {
  const rows = customers.length
    ? customers
      ...
      <td><a href="/clients/${c.id}">${esc(c.name)}</a>${c.archived ? ' <span class="tag">archived</span>' : ''}${
        c.invoice_delivery_mode === 'disabled'
          ? ' <span class="tag">invoice email disabled</span>'
          : c.invoice_delivery_mode === 'mail_app'
            ? ' <span class="tag">sends via mail app</span>'
            : ''
      }</td>
...
<div class="card">
  ...
  <form method="post" action="/clients">
    ...
    <label>Invoice delivery
      <select name="invoiceDeliveryMode">${deliveryModeOptions(tenantDefaultMode)}</select></label>
    <p class="muted" style="font-size:.85rem">Starts at this business's default (Settings). Change it
      per client any time — later default changes never rewrite an existing client.</p>
...
<div class="card">
  ...
  <form method="post" action="/clients/${customer.id}">
    ...
    <label>Invoice delivery
      <select name="invoiceDeliveryMode" required>${deliveryModeOptions(customer.invoice_delivery_mode)}</select></label>
    <p class="muted" style="font-size:.85rem">${
      customer.invoice_delivery_mode === 'disabled'
        ? `${hostedSendDenialReason('disabled')} The email action is hidden on this client's invoices; PDF download, print, and marking sent by hand still work.`
        : customer.invoice_delivery_mode === 'mail_app'
          ? "The invoice page prepares a PDF and a pre-filled draft for your own mail app; you still attach the PDF and hit send yourself, since a browser cannot do either of those for you."
          : "HourChit sends this client's invoices directly, with the invoice in the email body and a PDF attached."
    } Changing this affects only ${esc(customer.name)}.</p>
