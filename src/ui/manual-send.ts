...
(function () {
 ...
 var show=function(){if(fallbackOnly||skipNative||!pdf)reminder.hidden=false;details.hidden=false;el('manual-confirm').hidden=false;};
 ...
 async function prepare(){
  ...
  try {
   ...
   if(!canCompose()){say('Ready. Send copies the formatted body and opens an addressed email. Download and attach the PDF manually.');return;}
   ...
   if(version!==generation)return;pdf=null;base64=null;reminder.hidden=false;
 ...
 async function copyAndOpen(version){
  reminder.hidden=false;show();var rich=false;
  try {
 ...
 button.addEventListener('click',async function(event){
  ...
  try {
   if(pdf&&!fallbackOnly&&!skipNative&&native&&native.capabilities&&native.capabilities.invoiceHandoff===2&&typeof native.handoffInvoice==='function'){
    ...
    if(result.status==='unavailable')skipNative=true;else fallbackOnly=true;show();say('Mail handoff unavailable. Choose Send again to copy the body, then download and attach the PDF manually.');return;
   }
