...
/**
 * The web fallback for "send with your mail app".
 *
 * A browser cannot attach a file to a mailto: draft and cannot confirm
 * anything the operator's own mail app does after the draft opens, so this
 * page does the two things it honestly can: open a pre-filled draft, and
 * hand over the PDF to attach by hand. It never claims more than that, and
 * it never marks the invoice sent on its own -- only the operator saying so
 * does that, because nothing here can detect whether the mail actually went.
 */
...
<div class="card">
  ...
  <a href="${v.mailtoHref}" style="text-decoration:none"><button type="button">Open in your mail app</button></a>
</div>
