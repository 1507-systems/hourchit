...
export interface SettingsView {
  ...
  /**
   * This tenant's default mode for a newly created client. Only ever read at
   * client-creation time -- changing it here never rewrites an existing
...
<div class="card">
  <h2>Invoice delivery default</h2>
  <p class="muted">The mode a newly added client starts with. Every client also has its own explicit
    mode on its client page — changing this default here never changes an existing client.</p>
  <form method="post" action="/settings/invoice-delivery">
    <label>Default for new clients
      <select name="invoiceDeliveryDefault">${invoiceDeliveryModes
        .map(
          (m) =>
            `<option value="${m}"${m === v.invoiceDeliveryDefault ? ' selected' : ''}>${esc(INVOICE_DELIVERY_MODE_LABELS[m])}</option>`,
        )
        .join('')}</select></label>
  ...
  <form method="post" action="/settings/terms">
    ...
    <div id="agreed-with">
      <label>Agreed with — who assented, and when
        <input name="agreedWith" placeholder="e.g. Dana Reyes, Example Institutional Client, by email 2026-07-28"></label>
