...
// Kept in sync with src/domain/invoice-delivery.ts's invoiceDeliveryModes by
// hand -- this script runs standalone, before any TypeScript build step.
const VALID_INVOICE_DELIVERY_MODES = ['hosted', 'disabled', 'mail_app'];

const seed = profile.seed ?? {};
if (seed.customer) {
  const c = seed.customer;
  // Written explicitly rather than left to the column's own default: a
  // seeded client is a real client, and this is the one place a profile can
  // record that someone actually chose a delivery mode for it.
  const invoiceDeliveryMode = c.invoiceDeliveryMode ?? 'hosted';
  if (!VALID_INVOICE_DELIVERY_MODES.includes(invoiceDeliveryMode)) {
    console.error(
      `seed.customer.invoiceDeliveryMode must be one of ${VALID_INVOICE_DELIVERY_MODES.join(', ')}, got ${JSON.stringify(invoiceDeliveryMode)}`,
    );
    process.exit(1);
  }
  lines.push(
    `INSERT INTO customers (name, address, email, invoice_delivery_mode) ` +
      `VALUES (${q(c.name)}, ${q(c.address)}, ${q(c.email)}, ${q(invoiceDeliveryMode)});`,
  );
