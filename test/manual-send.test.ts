...
it('web skips share and PDF preparation, copies HTML and opens addressed mail',async()=>{
 ...
 await s.click();expect(share).not.toHaveBeenCalled();expect(write).toHaveBeenCalledOnce();
 expect(s.location.href).toBe('mailto:client%40example.test?subject=Invoice');
 expect(s.fetch.mock.calls.every(c=>!c[0].endsWith('/send')&&!c[0].endsWith('/email'))).toBe(true);
});
it('hides attachment reminder only for a usable native bridge',async()=>{
 const native={capabilities:{invoiceHandoff:2},handoffInvoice:vi.fn().mockResolvedValue({requestId:'test-id',status:'cancelled'})};
 const s=await setup({native});expect(s.elements['manual-attachment-reminder'].hidden).toBe(true);
 const incomplete=await setup({native:{capabilities:{invoiceHandoff:2}}});expect(incomplete.elements['manual-attachment-reminder'].hidden).toBe(false);
});
...
it('does not launch a duplicate while clipboard writing is active',async()=>{
