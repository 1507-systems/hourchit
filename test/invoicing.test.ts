...
describe('invoicing', () => {
  ...
  it('preserves the service, event name, and local work date on each time line', () => {
    const totals = buildInvoice(
      [
        {
          taskId: 1,
  ...
  it('omits mileage entirely when the client does not reimburse travel', () => {
    // The example tenant vs its institutional client: $125/hr flat, travel NOT
    // reimbursed. Billing it anyway is the failure this guards — it would not
    // surface until AP rejected the invoice weeks later.
    const totals = buildInvoice(
      [{ taskId: 1, taskName: 'Event Tech Management', rateCentsPerHour: 12500, seconds: 7200 }],
