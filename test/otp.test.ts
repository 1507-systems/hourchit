...
describe('login allowlist', () => {
  it('is case and whitespace insensitive', () => {
    expect(isAllowedLogin('  Owner@Example.Invalid ', ['owner@example.invalid'])).toBe(true);
  });
  ...
  it('refuses an address that is not configured', () => {
    ...
    expect(isAllowedLogin('attacker@example.com', ['owner@example.invalid'])).toBe(false);
  });

  it('refuses a near miss rather than matching loosely', () => {
    expect(isAllowedLogin('owner@example.invalid.evil.com', ['owner@example.invalid'])).toBe(false);
    expect(isAllowedLogin('owner@example.invali', ['owner@example.invalid'])).toBe(false);
  });
