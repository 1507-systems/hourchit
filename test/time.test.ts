...
describe('time', () => {
  ...
  it('adds minutes to an instant', () => {
    expect(addMinutesIso('2026-07-28T09:00:00.000Z', 90)).toBe('2026-07-28T10:30:00.000Z');
    expect(addMinutesIso('2026-07-28T23:45:00.000Z', 30)).toBe('2026-07-29T00:15:00.000Z');
    expect(addMinutesIso('2026-07-28T09:00:00.000Z', 0)).toBe('2026-07-28T09:00:00.000Z');
  });

  it('rejects an unreadable start instant', () => {
    expect(() => addMinutesIso('not-a-date', 30)).toThrow();
  });
});
