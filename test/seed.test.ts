...
describe('seed generator: invoice delivery mode', () => {
  it('defaults an unspecified mode to hosted, written explicitly rather than left to the schema', () => {
    const res = generate('zz-test', filled);
    expect(res.ok).toBe(true);
    expect(res.stdout).toMatch(/INSERT INTO customers \(name, address, email, invoice_delivery_mode\)/);
    expect(res.stdout).toContain("'hosted'");
  });

  it('writes an explicitly configured mode', () => {
    const res = generate('zz-test', {
      ...filled,
      seed: { ...filled.seed, customer: { ...filled.seed.customer, invoiceDeliveryMode: 'disabled' } },
    });
    expect(res.ok).toBe(true);
    expect(res.stdout).toMatch(/VALUES \([^)]+, 'disabled'\)/);
  });

  it('refuses an invalid mode rather than writing a value the column CHECK would later reject', () => {
    const res = generate('zz-test', {
      ...filled,
      seed: { ...filled.seed, customer: { ...filled.seed.customer, invoiceDeliveryMode: 'sms' } },
    });
    expect(res.ok).toBe(false);
    expect(res.stderr).toMatch(/invoiceDeliveryMode must be one of/);
    expect(res.stdout).not.toContain('INSERT INTO');
  });
});
