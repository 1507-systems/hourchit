...
describe('the invoice email body', () => {
  it('carries the whole invoice, not a link to it', () => {
    ...
    expect(t).toContain('Event Tech Management');
    expect(t).toContain('Event Tech Management - 9/9');
    expect(t).toContain('Awards Ceremony - GL 5678');
  ...
  it('styles the HTML inline, because clients discard <head>', () => {
    ...
    expect(html).toContain('font-size:15px;font-weight:700">Event Tech Management - 9/9');
    expect(html).toContain('Awards Ceremony - GL 5678');

    const withoutCode = invoiceEmailText(view({ lines: [{ ...lines[0], charge_code: null }] }));
    expect(withoutCode).toContain('Awards Ceremony');
    expect(withoutCode).not.toContain('Awards Ceremony - ');
  });
...
describe('the intro line above the invoice', () => {
  it('appears in the text part before the invoice header', () => {
    const t = invoiceEmailText(view());
    expect(t).toContain('please find your invoice from Tarnsby A/V Services LLC');
    expect(t.indexOf('please find your invoice')).toBeLessThan(t.indexOf('INVOICE TBY-0008'));
  });

  it('appears in the HTML part before the invoice header table', () => {
    const html = invoiceEmailHtml(view());
    expect(html).toContain('please find your invoice from Tarnsby A/V Services LLC');
    expect(html.indexOf('please find your invoice')).toBeLessThan(html.indexOf('INVOICE'));
  });

  it('escapes a client name in the intro rather than injecting it', () => {
    const html = invoiceEmailHtml(view({ customer: { name: '<script>alert(1)</script>' } }));
...
describe('the sent-on-behalf-of disclosure', () => {
  it('names the BUSINESS, not HourChit, as who the message is from', () => {
    ...
    // white-labelled booking-service convention: the business first,
