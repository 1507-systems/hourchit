...
describe('bareAddress', () => {
  it('strips a display name', () => {
    expect(bareAddress('Dana Reyes <dreyes@client.example>')).toBe('dreyes@client.example');
  });
  it('lowercases, because mail addresses are compared case-insensitively', () => {
    expect(bareAddress('DReyes@Client.EXAMPLE')).toBe('dreyes@client.example');
  });
});
...
describe('addressList', () => {
  ...
  it('does not split on a comma inside a quoted display name', () => {
    // "Reyes, Dana" is one recipient. Splitting on the comma invents a second.
    expect(addressList('"Reyes, Dana" <dreyes@client.example>, t@x.com'))
      .toEqual(['dreyes@client.example', 't@x.com']);
  });
...
describe('resolveSender', () => {
  const contacts = [
    { id: 1, customer_id: 5, email: 'dreyes@client.example' },
    { id: 2, customer_id: 5, email: 'thale@client.example' },
  ];

  it('attributes a known sender to their contact and customer', () => {
    expect(resolveSender('Dana Reyes <dreyes@client.example>', contacts))
      .toEqual({ contactId: 1, customerId: 5 });
  });

  it('matches case-insensitively', () => {
    expect(resolveSender('DREYES@CLIENT.EXAMPLE', contacts).contactId).toBe(1);
  });
  ...
  it('still attributes a DEACTIVATED contact', () => {
    ...
    const withGone = [...contacts, { id: 3, customer_id: 5, email: 'gone@client.example' }];
    expect(resolveSender('gone@client.example', withGone).contactId).toBe(3);
  });
});
