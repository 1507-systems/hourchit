...
const CUSTOMER = {
  ...
  notes: '',
  notice_days: 60,
  invoice_delivery_mode: 'hosted' as InvoiceDeliveryMode,
};
...
describe('POST /invoices/:id/email', () => {
  it('refuses to email a canceled invoice', async () => {
    ...
    expect(res.headers.get('location')).toContain('cannot%20be%20emailed');
  ...
  it('sends from the TENANT address, not the HourChit apex', async () => {
    // The invoice is the tenant billing their client, not HourChit talking to
    // the tenant. Sending it from noreply@hourchit.app puts a vendor the client
    // has never heard of on a demand for money.
    const mail = recordingMail();
    const { e } = env({
      EMAIL: mail.binding as never,
  ...
  it('marks it sent and logs it outbound on success', async () => {
    ...
    await app.request('/invoices/8/email', { method: 'POST', ...AUTH }, e);
    expect(mail.sent[0].to).toBe('client@example.invalid');
...
describe('hosted send is refused for a client not in hosted mode', () => {
  // A hidden button is a convenience, not a boundary: a direct request to
  // either route must resolve the same policy the client page shows, so
  // there is exactly one place "can this client be hosted-emailed" is
  // decided, not one for the button and a different one for the route.
  it('GET /invoices/:id/email refuses with 403 when disabled', async () => {
    const { e } = env({}, { customer: { ...CUSTOMER, invoice_delivery_mode: 'disabled' } });
    const res = await app.request('/invoices/8/email', { method: 'GET', ...AUTH }, e);
    expect(res.status).toBe(403);
    expect(await res.text()).toBe('Invoice email delivery is disabled for this business.');
  });

  it('POST /invoices/:id/email refuses with 403 when disabled, and sends nothing', async () => {
    const mail = recordingMail();
    const { e } = env(
      { EMAIL: mail.binding as never, BROWSER: fakeBrowser() as never },
      { customer: { ...CUSTOMER, invoice_delivery_mode: 'disabled' } },
    );
    const res = await app.request('/invoices/8/email', { method: 'POST', ...AUTH }, e);
    expect(res.status).toBe(403);
    expect(mail.sent).toHaveLength(0);
  ...
  it('GET /invoices/:id/email refuses with 403 when the client uses their own mail app', async () => {
    const { e } = env({}, { customer: { ...CUSTOMER, invoice_delivery_mode: 'mail_app' } });
    const res = await app.request('/invoices/8/email', { method: 'GET', ...AUTH }, e);
    expect(res.status).toBe(403);
  });

  it('POST /invoices/:id/email refuses with 403 when the client uses their own mail app, and sends nothing', async () => {
    const mail = recordingMail();
    const { e } = env(
      { EMAIL: mail.binding as never, BROWSER: fakeBrowser() as never },
      { customer: { ...CUSTOMER, invoice_delivery_mode: 'mail_app' } },
    );
    const res = await app.request('/invoices/8/email', { method: 'POST', ...AUTH }, e);
    expect(res.status).toBe(403);
    expect(mail.sent).toHaveLength(0);
  ...
  it('still allows a hosted-mode client through unaffected', async () => {
    const mail = recordingMail();
